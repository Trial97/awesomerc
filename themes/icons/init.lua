...
-- Icons directory
local dir = gfs.get_configuration_dir() .. "/themes/icons/"

local function getLevelIcon(name, level)
  if level == 0 then
    return name .. "-muted"
  elseif level < 100 / 3 then
    return name .. "-low"
  elseif level < 100 / 3 * 2 then
    return name .. "-medium"
  else
    return name .. "-high"
  end
