...
local function buttons()
    awful.mouse.append_global_mousebindings({
        awful.button({}, 1, function() awesome.emit_signal('module::menu:toggle', false) end),
        awful.button({}, 3, function() awesome.emit_signal('module::menu:toggle', true) end),
        awful.button({}, 2, function() awful.util.spawn(apps.appmenu) end),
        awful.button({ 'Control' }, 2, function() awesome.emit_signal('module::exit_screen:show') end),
        awful.button({ 'Shift' }, 2, function() awesome.emit_signal('widget::blue_light:toggle') end),
    })
...
local function keybindings()
    ...
    awful.keyboard.append_global_keybindings({
        ...
        awful.key({ modkey, }, "w", function() awesome.emit_signal('module::menu:toggle', true) end,
            { description = "show main menu" }),
        awful.key({ modkey, }, 'l', function() awful.spawn(apps.lock, false) end,
            { description = 'lock the screen' }),
        awful.key({ altkey, }, "Tab", function() awesome.emit_signal("bling::window_switcher::turn_on") end,
            { description = "Window Switcher" }),
        awful.key({ modkey }, "Escape", revelation,
            { description = "Window Switcher" }),
        awful.key({ modkey, 'Control', "Shift" }, 't', function() awesome.emit_signal('module::rules:titlebar') end,
    ...
    awful.keyboard.append_global_keybindings({
        ...
        awful.key({ modkey, 'Shift' }, 'e', function() awful.spawn(apps.file_manager) end,
            { description = 'open default file manager' }),
        awful.key({ modkey, }, 'f', function() awful.spawn(apps.web_browser) end,
        ...
        awful.key({ modkey, }, 'c', function() awful.spawn(apps.ide) end,
            { description = 'open default IDE' }),
        awful.key({ modkey, }, 'g', function() awful.spawn(apps.git) end,
            { description = 'open default git client' }),
    })
    ...
    awful.keyboard.append_global_keybindings({
        ...
        awful.key({}, 'Print', function()
            awful.spawn.easy_async_with_shell(apps.full_screenshot, function()
            end)
        end, { description = 'fullscreen screenshot' }),
        awful.key({ 'Shift' }, 'Print', function()
            awful.spawn.easy_async_with_shell(apps.area_screenshot, function()
            end)
    ...
    awful.keyboard.append_global_keybindings({
        group = "tag",
        awful.key({ modkey, "Control" }, 'w', awful.tag.viewprev,
