...
local update_gap_and_shape = function(t)
    ...
    if (current_layout == awful.layout.suit.max) then
        ...
        for _, c in ipairs(t:clients()) do
            if not c.floating or not c.round_corners or c.maximized or
                c.fullscreen then
...
tag.connect_signal('property::layout', function(t) update_gap_and_shape(t) end)

-- Change tag's client's shape and gap on move to tag
tag.connect_signal('tagged', function(t) update_gap_and_shape(t) end)

-- Focus on urgent clients
awful.tag.attached_connect_signal(nil, 'property::selected', function()
    for c in awful.client.iterate(function(c) return awful.rules.match (c, {urgent = true}) end) do
