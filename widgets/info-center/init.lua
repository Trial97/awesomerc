...
awesome.connect_signal('widgets::info_center:toggle', function(open)
    ...
    for s in screen do 
        if s.info_center == nil then return end
        if s ~= focused then 
...
return function (s)
    ...
    local panel = awful.popup {
        widget  = {
            ...
            shape  = function(cr, w, h) gears.shape.rounded_rect(cr, w, h, beautiful.groups_radius) end,
    ...
    function panel:toggle(open)
        if open == self.opened then return end
    ...
    panel.backdrop:buttons({awful.button({}, 1, nil, function() awesome.emit_signal('widgets::info_center:toggle') end)})
    return panel
end
