...
local function get_icon(battery_percentage, status)
  ...
  if (status == 'fully-charged' or status == 'charging') and battery_percentage == 100 then -- Fully charged
    icon_name = icon_name .. '-' .. 'fully-charged'
  elseif (battery_percentage > 0 and battery_percentage < 10) and status == 'discharging' then -- Critical level warning message
  ...
  awful.spawn.easy_async_with_shell(
    ...
    function(stdout)
      ...
      if icon_name == 'battery-alert-red' then
        local now = os.time()
        if os.difftime(now, last_check) > 300 or notify_critcal then
          last_check = now
