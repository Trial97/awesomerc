...
local msg_table        = {
    ...
    'Stay out of trouble.', 'I’m out of here.', 'Yamete, onii-chan~. UwU',
    'Okay...bye, fry guy!', 'Peace out!', 'Peace out, bitch!',
    'Gotta get going.', 'Out to the door, dinosaur.',
    'Don\'t forget to come back!', 'Smell ya later!', 'In a while, crocodile.',
    'Adios, amigo.', 'Begone!', 'Arrivederci.', 'Never look back!',
    'So long, sucker!', 'Au revoir!', 'Later, skater!',
    'That\'ll do pig. That\'ll do.', 'Happy trails!', 'Smell ya later!',
    'See you soon, baboon!', 'Bye Felicia!', 'Sayonara!', 'Ciao!',
    'Well.... bye.', 'Delete your browser history!', 'See you, Space Cowboy!',
...
local function suspend_command() awful.spawn.with_shell('systemctl suspend') end

local function logout_command() awesome.quit() end

local function lock_command() awful.spawn.with_shell(lock_cmd) end

local function poweroff_command() awful.spawn.with_shell('poweroff') end

local function reboot_command() awful.spawn.with_shell('reboot') end
...
local function create_base_exit_screen(s)
    ...
    exit_screen:buttons({
        awful.button({}, 2, function() awesome.emit_signal('module::exit_screen:hide') end),
        awful.button({}, 3, function() awesome.emit_signal('module::exit_screen:hide') end),
    })
    return exit_screen
end
...
gears.timer {
    ...
    call_now    = true,
    single_shot = true,
    callback    = function() awesome.emit_signal('module::exit_screen:redraw') end
}
