...
local volume        = osd("volume", "Volume", icons.volume, function(level, pressed)
    if pressed then spawn('amixer -D pulse sset Master ' .. level .. '%', false) end
    awesome.emit_signal('widgets::volume:update', level)
end)

local mic           = osd("mic", "Microphone", icons.mic, function(level, pressed)
    if pressed then spawn('amixer -D pulse sset Capture ' .. level .. '%', false) end
end)

local bri           = osd("brightness", "Brightness", icons.brightness, function(level, pressed)
    if pressed then spawn('light -S ' .. math.max(level, 5), false) end
end)
...
return function(tags)
    ...
    tag.connect_signal('property::selected', function() update_bars_visibility() end)

    client.connect_signal('property::fullscreen', function(c)
        if c.first_tag then c.first_tag.fullscreen_mode = c.fullscreen end
